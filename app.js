...
import { loginSchema, fleetSchema, paymentReportSchema, paymentVerifySchema, driverProfileSchema, notificationReadSchema, aiAnalyzeSchema, forgotPasswordSchema, resetPasswordSchema, whatsappSendSchema } from './schemas.js';
...
app.post('/api/auth/login', loginLimiter, async (req, res) => {
    ...
    if (r.rows.length > 0) {
        ...
        if (match) {
            ...
            const token = jwt.sign({ id: user.id, email: user.email, role: user.role, tenant_id: user.tenant_id }, process.env.JWT_SECRET || 'aurum-secret-key-change-in-prod', { expiresIn: '8h' });
