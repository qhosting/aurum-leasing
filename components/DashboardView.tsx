...
const chartData = [
  { name: 'Lun', flow: 1200 }, { name: 'Mar', flow: 2100 }, { name: 'Mie', flow: 800 },
  { name: 'Jue', flow: 1600 }, { name: 'Vie', flow: 2400 }, { name: 'Sab', flow: 3100 }, { name: 'Dom', flow: 1800 },
];
...
const DashboardView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      setIsLoading(true);
      const data = await persistenceService.getArrendadorStats();
      setMetrics(data);
      setIsLoading(false);
    };
    fetchStats();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900 p-8 rounded-[3rem] text-white relative overflow-hidden shadow-2xl group">
             ...
             <p className="text-slate-400 text-xs font-bold uppercase tracking-widest leading-relaxed">Gemini ha detectado anomalías en contratos activos.</p>
